...
export class UpcomingInterviewsComponent implements OnInit {
  public readonly MODE = 'upcoming';
  public avatar = 'ADMIN';

  public noDataMessage = 'No upcoming interview found.';
  ...
  public downloadCsv(): void {
    ...
    this.reportService.getInterviewReportFile(this.MODE, this.avatar).subscribe(data => {
      ...
      const fileName = `upcoming-interviews_${format(new Date(), 'MM-DD-YYYY')}.csv`;
    ...
    this.reportService.getInterviewReport(param).subscribe((res: PaginatedApi<InterviewReport>) => {
      ...
      error => {
        ...
        if (isApiError(error)) {
          ...
          this.error = 'Error fetching upcoming interview.';
        }
      });
  }
}
